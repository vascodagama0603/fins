...
class fins:
    ...
    def selectExchangeFunc(self, adr, type):
        outdata = [-999]
        if type == "INT16":
            data = self.read(adr, 1)
            outdata = self.toInt16(data)
        elif type == "UINT16":
            data = self.read(adr, 1)
            outdata = self.toUInt16(data)
        elif type == "INT32":
            data = self.read(adr, 2)
            outdata = self.toInt32(data)
        elif type == "UINT32":
            data = self.read(adr, 2)
            outdata = self.toUInt32(data)
        elif type == "INT64":
            data = self.read(adr, 4)
            outdata = self.toInt64(data)
        elif type == "UINT64":
            data = self.read(adr, 4)
            outdata = self.toUInt64(data)
        elif type == "FLOAT":
            data = self.read(adr, 4)
            outdata = self.toFloat(data)
        elif type == "DOUBLE":
            data = self.read(adr, 4)
            outdata = self.toDouble(data)
        elif type == "STRING":
            data = self.read(adr, 4)
            outdata = self.toString(data)
        return outdata
